...
const api = {
  ...
  isTokenExpired(token) {
    ...
    if (token.startsWith("local_auth_valid_")) return false;
    // Wipe old legacy mock bypass tokens from previous versions
    if (token === "local_session_token" || token.startsWith("local_session_token")) return true;
    if (!token.includes(".")) return true; // JWTs must have 3 parts separated by dots
    try {
